...
===SLUG===
claude-code-3-layer-defense

===META===
Claude Code で「AIが.envを読んだ」「本番にforce pushされた」が普通に起きる。10分のコピペで deny / ask / hooks の3層防御を構築する完全ガイド。
...
{{caution:本記事は2026年4月18日時点の情報です。Claude Codeのhooks仕様・permissions書式は公式ドキュメント（[code.claude.com/docs](https://code.claude.com/docs/)）と照合済み。ただしClaude Codeは活発に更新されているため、仕様が変わる可能性があります。実施前に公式ドキュメントで最新情報を確認してください。}}

**【この記事で分かること】**

- Claude Code を設定なしで使うリスク（3つの事故パターン）
- deny / ask / hooks の3層防御設計の考え方
- 10分で完了するコピペ手順（Step 0〜7）
- 90個超の deny / ask ルール設定ファイル
- Bashガードスクリプト・機密ファイル保護Pythonフック
- 自分のクラウドサービス（Supabase/Vercel/AWS 等）を追加するカスタマイズ例
...
### Q: この設定を入れたら、Claude Code が使いにくくなりませんか？
A: 「元に戻せない操作」だけを deny、「確認すれば問題ない操作」を ask に分けているので、日常的な操作はほぼノンストレスで動きます。実際に僕は毎日これで開発していますが、止まるのは「本当に止まってほしい時」だけです。

### Q: チームメンバーに展開するにはどうすればいい？
A: プロジェクトの `.claude/settings.json` をGitにコミットします。プロジェクトを clone したメンバー全員に同じ制限が自動で適用される状態になります。チーム全体のセキュリティは、一番ルーズなメンバーの設定レベルで決まるので、個人任せにしないでください。

### Q: Claude Codeの仕様が変わったらどうすれば？
A: 公式ドキュメント（code.claude.com/docs）の hooks / permissions セクションを確認して、この記事の記述と照合してください。大きな変更があれば本記事も更新します。LINE公式で更新通知を受け取れます。
...
### 📱 個人で学びたい方へ
Claude Code・AI活用の情報・実務テンプレートをLINE公式で配信しています。ラボの棚（資料集）のアクセス権も届きます。
→ [LINE公式を追加]({{LINE_URL}})

===TAGS===
Claude Code,AIセキュリティ,hooks,deny,開発環境
